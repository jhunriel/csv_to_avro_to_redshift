...
-------####################### STORED PROCEDURE #########################-----------------------
create or replace procedure sp_engagement(_from timestamp,_to timestamp, rs_out INOUT refcursor)
as $$
begin
	open rs_out for select count(*) as No_of_Transactions
		...
		,floor_date_sgt::time as Time
	from campaign_transactions
	where transaction_date_sgt between _from and _to
	group by 2,3
	order by 1 desc
    ;
end;
...
----------------------------- Navigate ------------------------------------
call sp_engagement('2019-08-12 19:00:00','2019-08-16 19:00:00','mycursor');
FETCH FOWARD 100 FROM mycursor;
commit;
--------------------------------------------------------------------------
